...
if (isset($_GET['search'])) {
    $student_number = trim($_GET['search']);
    ...
    // checks if student exists so that we know if we want to update or submit a registration
    if ($result->num_rows > 0 && ($row = $result->fetch_assoc())) {
        $is_update = true;
    } else {
        // gives alert and redirects if student not found
...
<body>
    <main>
        ...
        <div id="form-container" class="containers">
            ...
            <form method="POST" id="student-form" action=<?php echo $is_update ? "update_feature.php" : "insert_delete_feature.php"; ?> enctype="multipart/form-data">
                <!-- FORM MAIN BODY -->
                <div id="form-body">
                    ...
                    <div id="academic-info-panel" class="gen-sections">
                        ...
                        <p>Graduating this year? (leave unchecked if not)<span class="required">*</span>
                            <input type="checkbox" id="grad-status-input" name="graduation_status" <?php echo ($row['Graduation_status'] == 1) ? 'checked' : ''; ?>>
                        </p>
                    </div>
